...
Распакуйте архив с сервером в любую папку. После этого нужно создать подпапку World, в которой будут храниться все данные игры.

Создайте в папке World файл Settings.json (его можно открыть простым блокнотом). Вот пример его содержимого:
```
...
IsModsWhitelisted - если используется точное соответствие модов, то установить true, иначе false.

ModsDirectory - если IsModsWhitelisted установлено в истину, то тут папка которая будет скопирована каждому клиенту в его папку Mods, иначе значение игнорируется. Подробнее ниже.

ModsConfigsDirectoryPath - если IsModsWhitelisted установлено в истину, то аналогично ModsDirectory следует указать папку с настройками модов. Подробнее ниже.

DisableDevMode - будет ли отключаться режим разработчика, когда игроки заходят онлайн.

MinutesIntervalBetweenPVP - после нападения на одного игрока другим, на него не могут нападать заданное количество минут. Можно поставить, например, 0 или 20.
...
EnablePVP - Включить режим нападения игроков друг на друга онлайн с ограниченным управлением
...
IgnoredLocalConfigFiles - Имена игнорируемых файлов настроек. Остальные будут синхронизированы с папкой на сервере указанной в ModsConfigsDirectoryPath, если IsModsWhitelisted установлено в истину.

IgnoredLocalModFiles - Окончание игнорируемых имен файлов из папки с модами. Остальные будут синхронизированы с папкой на сервере указанной в ModsDirectory, если IsModsWhitelisted установлено в истину. 
...
Перед запуском проверьте, что у вас установлен .NET Core 3.1: https://dotnet.microsoft.com/download/dotnet-core/current/runtime

Запустите ServerOnlineCity.exe. Должна открыться консоль, которая и является сервером. Вывод в неё дублируется в лог файлы в папке World.

Для того, что бы люди могли подключится к нему, нужно открыть порт 19019 (или тот который указан в Port).

Когда сервер запустится, в папке с ним будет создана папка "World". Первый человек, который зарегистрируется на сервере через игру, будет считаться администратором. Ему доступны специальные команды и режим разработчика, даже если он выключен настройками сервера. При первом входе/регистрации администратору будет предложено выбирать сиид мира, заполненость мира в процентах и сложность игры (просто/нормально/сложно). Сценарий и рассказчик всегда одинаковы при старте. После этого, игре понадобиться несколько минут, после чего вас вернёт в главное меню. Всё готово! Можно заходить снова и как обычный игрок создавать поселение.

В *nix системах запуск должен происходить также без проблем, т.к. сервер написан на .NET Core 3.1.
...
Подробнее как настроить режим точного соответствия модов.

Чтобы правильно всё установить настройте свой RimWorld так, чтобы все моды брались из папки Mods (ни одного мода из Steam!). Проверьте, что всё нормально работает в обычной игре. И скопируйте свою папку Mods на сервер. Как минимум у вас должно быть три папки (внутри Mods): Harmony, HugsLib, OnlineCity.

После этого скопируйте на сервер в папку World файл ModsConfig.xml из игры 
%appdata%\..\LocalLow\Ludeon Studios\RimWorld by Ludeon Studios\Config
У меня, например, он лежит тут:
"c:\Users\Ant\AppData\LocalLow\Ludeon Studios\RimWorld by Ludeon Studios\Config\ModsConfig.xml" 

В настройке ModsConfigsDirectoryPath должна указываться папка на сервере, куда нужно скопировать файлы настроек модов. То есть все остальные файлы из той же папки где находиться ModsConfig.xml
