...
namespace ServerOnlineCity.Model
{
    ...
    public class FMailIncident : IFunctionMail
    {
        ...
        public bool Run(ServiceContext context)
        {
            ...
            var countInOrder = context.Player.FunctionMails.Count(m => m != this && ((m as FMailIncident)?.NumberOrder ?? 0) == NumberOrder).ToString();
        ...
        private long CalcDelayStart()
        {
            return NumberOrder == 3 && Mail.IncidentMult >= ServerManager.ServerSettings.GeneralSettings.IncidentMaxMult / 2
                ? ServerManager.ServerSettings.GeneralSettings.IncidentAlarmInHours * 2500 /*ServerManager.ServerSettings.GeneralSettings.IncidentTickDelayBetween / 2f*/
                : 0;
        }
