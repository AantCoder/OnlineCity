...
namespace ServerOnlineCity.ChatService
{
    internal sealed class SetGrantsForUser : IChatCmd
    {
        ...
        public ModelStatus Execute(ref PlayerServer player, Chat chat, List<string> argsM, ServiceContext context)
        {
            ...
            if ((anotherPlayer.Public.Grants & Grants.SuperAdmin) == Grants.SuperAdmin)
            {
                return _chatManager.PostCommandPrivatPostActivChat(ChatCmdResult.UserNotFound, myLogin, chat, $"Can't change grants {argsM[1]}");
            }
            ...
            if ("add".Equals(subCmd) || "revoke".Equals(subCmd))
            {
                ...
                if (newGrants.Equals(Grants.NoPermissions)
                    || newGrants.Equals(Grants.SuperAdmin)
                    || newGrants.Equals(Grants.DiscordBot))
                {
